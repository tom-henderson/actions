...
if [[ ! -e "${GITHUB_WORKSPACE}/${ARGS}-version.json" ]]; then
	echo "ERROR: ${GITHUB_WORKSPACE}/${ARGS}-version.json not found, unable to determine version."
	exit 1
fi

jq . "${GITHUB_WORKSPACE}/${ARGS}-version.json"
CURRENT=$(jq --raw-output .current "${GITHUB_WORKSPACE}/${ARGS}-version.json")
NEXT=$(jq --raw-output .next "${GITHUB_WORKSPACE}/${ARGS}-version.json")
